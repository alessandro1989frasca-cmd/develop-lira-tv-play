...
function isItalyCEST(year: number, month1: number, day: number): boolean {
  if (month1 >= 4 && month1 <= 9) return true;
  if (month1 < 3 || month1 > 10) return false;
  if (month1 === 3) return day >= getLastSundayOfMonth(year, 3);
  if (month1 === 10) return day < getLastSundayOfMonth(year, 10);
  return false;
}
...
function parseXMLTVDate(dateStr: string): Date {
  ...
  if (offsetMinutes === 60 && isItalyCEST(year, month + 1, day)) {
    offsetMinutes = 120;
  }
  const date = new Date(Date.UTC(year, month, day, hour, minute, second) - offsetMinutes * 60000);
  return date;
}
...
async function upsertScheduleToDB(programs: Array<{ id: string; title: string; description: string; startTime: string; endTime: string; category: string }>): Promise<number> {
  ...
  console.log(`[Schedule] Filtering to today: ${todayPrograms.length} of ${programs.length} programs`);
  ...
  else console.log("[Schedule] Deleted past schedule entries");
...
async function getCachedFromDB() {
  const now = new Date();
  const utcMidnightToday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())).toISOString();

  const { data, error } = await supabase
    .from("cached_schedule")
    .select("*")
    .gte("end_time", utcMidnightToday)
...
export const scheduleRouter = createTRPCRouter({
  forceRefresh: publicProcedure.mutation(async () => {
    ...
    if (freshPrograms.length > 0) {
      const saved = await upsertScheduleToDB(freshPrograms);
      return { saved, source: 'xml' as const };
    }
    return { saved: 0, source: 'empty' as const };
  }),
  ...
  getSmartSchedule: publicProcedure.query(async () => {
    ...
    try {
      ...
      if (freshPrograms.length > 0) {
        ...
        const { rows: updatedRows } = await getCachedFromDB();
        if (updatedRows.length > 0) {
          console.log(`[Schedule] Refreshed cache with ${updatedRows.length} programs from XML`);
          return { programs: updatedRows, source: 'xml' as const };
        }
        const now = new Date();
        const utcMidnight = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
        const filteredFresh = freshPrograms.filter(p => new Date(p.endTime) >= utcMidnight);
        console.log(`[Schedule] DB read returned empty after upsert, returning ${filteredFresh.length} filtered in-memory programs`);
        return { programs: filteredFresh, source: 'xml' as const };
      }
    } catch (err) {
