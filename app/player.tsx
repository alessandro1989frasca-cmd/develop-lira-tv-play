...
export default function PlayerScreen() {
  ...
  const isClosingRef = useRef(false); /* guardia atomica — sincrona, no race condition */
  ...
  // Carica posizione salvata e pulisce voci vecchie (>30 giorni)
  useEffect(() => {
    if (!videoId) return;
    const THIRTY_DAYS_MS = 30 * 24 * 60 * 60 * 1000;
    AsyncStorage.getItem(`video_resume_${videoId}`)
      .then(raw => {
        if (!raw) return;
        const { position, savedAt } = JSON.parse(raw) as { position: number; duration?: number; savedAt: number };
        if (Date.now() - savedAt < THIRTY_DAYS_MS && position > 5) {
          resumePositionRef.current = position;
        } else {
          AsyncStorage.removeItem(`video_resume_${videoId}`).catch(() => {});
        }
      })
      .catch(() => {});
    // Pulizia voci scadute
    AsyncStorage.getAllKeys()
      .then(keys => {
        keys.filter(k => k.startsWith('video_resume_')).forEach(key => {
          AsyncStorage.getItem(key).then(raw => {
            if (!raw) return;
            const { savedAt } = JSON.parse(raw) as { savedAt: number };
            if (Date.now() - savedAt > THIRTY_DAYS_MS) AsyncStorage.removeItem(key).catch(() => {});
          }).catch(() => {});
        });
      })
      .catch(() => {});
  ...
  const player = useVideoPlayer(
    { uri: videoUrl, headers: { Referer: 'LiraTVPlay' } },
    (p) => {
      p.loop = false;
  ...
  useEffect(() => {
    if (playerStatus === 'error') {
      ...
      captureWarn(new Error(errMsg), 'player', 'player_status_error');
      setPlayerError(true);
      setIsStalled(false);
      setIsLoading(false);
      ...
      // Resume: seek alla posizione salvata solo al primo readyToPlay
      if (resumePositionRef.current !== null && !hasResumedRef.current) {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            setTimeout(() => {
              if (isMountedRef.current) setShowResumeBanner(false);
            }, 3000);
          } catch {}
        }, 400);
      }
  ...
  // Salva posizione ogni 10s — non salva se quasi finito (>92%) o ai primi 5s
  useEffect(() => {
    if (!videoId) return;
    const interval = setInterval(() => {
      if (!player || videoFinishedRef.current || !isMountedRef.current) return;
      const position = player.currentTime ?? 0;
      const duration = player.duration ?? 0;
      if (position < 5) return;
      if (duration > 0 && position / duration > 0.92) return;
      AsyncStorage.setItem(`video_resume_${videoId}`, JSON.stringify({
        position,
        duration,
        savedAt: Date.now(),
      })).catch(() => {});
  ...
  const handleClose = useCallback(async () => {
    /* Guardia ATOMICA su ref — sincrona, nessuna race condition tra doppio tap
       (tasto fisico + bottone X in simultanea non possono entrambi passare) */
    if (isClosingRef.current) {
      console.log('[Player] handleClose già in corso, ignorato');
      return;
    }
    isClosingRef.current = true;
    setIsClosing(true); /* aggiorna UI: nasconde i controlli */
    ...
    if (Platform.OS !== 'web') {
      try {
        ...
        /* Aspetta il cambio di orientamento EFFETTIVO invece di un timeout fisso.
           Su dispositivi lenti la rotazione può richiedere 400-500ms: un timeout
           fisso di 300ms provocherebbe un flash landscape sulla schermata precedente.
           Fallback a 1s nel caso raro in cui l'evento non arrivi. */
        await new Promise<void>((resolve) => {
          ...
          /* Fallback: se l'evento non arriva entro 1s procedi comunque */
          const fallback = setTimeout(doResolve, 1000);
        });
      } catch (e) {
    ...
    router.back();
  }, [player]); /* isClosingRef è una ref, non va nelle dipendenze */

  /* Reset ref all'unmount (se il componente viene smontato per altra via) */
  useEffect(() => {
    return () => { isClosingRef.current = false; };
  }, []);

  /* Tasto fisico "indietro" Android → usa handleClose per attendere
     la rotazione portrait prima di tornare alla schermata precedente */
  useEffect(() => {
    ...
    <View style={containerStyle}>
      ...
      <View style={videoContainerStyle}>
          ...
          <View style={styles.errorOverlay}>
            ...
            <View style={styles.errorButtonRow}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={styles.closeErrorButtonText}>Chiudi</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
        ...
        {showResumeBanner && (
          <View style={styles.resumeBanner} pointerEvents="none">
            <Text style={styles.resumeBannerText}>
              ▶ Ripreso da {formatTime((resumePositionRef.current ?? 0) * 1000)}
            </Text>
          </View>
        )}
        ...
        {showControls && !isLoading && (
          <>
            <View style={[styles.topControls, { paddingTop: isFullscreen ? Math.max(insets.top, 16) : 8 }]}>
              {isFullscreen && (
                <TouchableOpacity
                  onPress={handleClose}
            ...
            <View style={[styles.bottomControls, { paddingBottom: isFullscreen ? Math.max(insets.bottom + 8, 16) : 12 }]}>
