...
import { Newspaper, RefreshCw, X, WifiOff, SlidersHorizontal, Play, Maximize2 } from 'lucide-react-native';
...
function NewsVideoPlayer({ videoUrl, thumbnail }: NewsVideoPlayerProps) {
  ...
  return (
    <View style={styles.videoContainer}>
      ...
      <TouchableOpacity style={styles.fullscreenButton} onPress={handleFullscreen} activeOpacity={0.8}>
    ...
    <Animated.View style={[styles.articleModal, { opacity: fadeAnim }]}>
      <Animated.View style={[styles.articleModalContent, { transform: [{ translateY: slideAnim }] }]}>
        ...
        <ScrollView style={styles.articleModalBody} showsVerticalScrollIndicator={true}>
          {article.videoUrl ? (
            <NewsVideoPlayer videoUrl={article.videoUrl} thumbnail={article.thumbnail} />
          ) : (
            <Image
