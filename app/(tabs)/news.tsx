...
import { Newspaper, RefreshCw, X, WifiOff, SlidersHorizontal, Play, Pause, Maximize2, Minimize2, Bell, Share2 } from 'lucide-react-native';
  ...
  useEffect(() => {
    const initNotifState = async () => {
      ...
      if (status === 'granted' && !enabled) {
        const result = await requestAndRegisterNotifications(true);
        if (result === 'granted') {
...
function NewsVideoPlayer({ player, isPlaying, started, onStarted, thumbnail, isFullscreen, onFullscreenChange, containerStyle }: NewsVideoPlayerProps) {
  ...
  const bottomInset = isFullscreen ? Math.max(24, insets.bottom + 8) : 8;
  ...
  return (
    <View style={containerStyle}>
      ...
      {started && showControls && (
        <View style={[styles.newsBottomBar, { paddingBottom: bottomInset, paddingRight: isFullscreen ? Math.max(24, insets.right + 8) : 0 }]}>
...
function ArticleModal({ article, onClose }: ArticleModalProps) {
  ...
  const handleShare = useCallback(async () => {
    try {
      await Share.share(
        Platform.OS === 'ios'
          ? { message: article.title, url: article.link, title: 'Lira TV' }
          : { message: `${article.title}\n${article.link}`, title: 'Lira TV' }
      );
    } catch {}
      ...
      >
        ...
        >
          ...
          >
            ...
            <TouchableOpacity onPress={handleShare} style={styles.closeButton} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
              <Share2 color="#F5C518" size={20} />
            </TouchableOpacity>
