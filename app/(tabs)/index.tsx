...
import { EventStream } from '@/lib/appConfig';
import Colors from '@/constants/colors';
import { fetchRSSFeed, fetchSchedule, getCurrentProgram, fetchFeaturedPrograms, fetchActivePolls, submitPollVote, FeaturedProgram, PollData } from '@/utils/dataService';
      ...
      >
        {heroProgram ? (
          ...
          >
            ...
            />
            <LinearGradient
              colors={['transparent', 'rgba(10,14,26,0.75)', Colors.dark.background]}
              locations={[0, 0.45, 1]}
        ...
        {appConfig.matchWidgetEnabled && appConfig.matchData && (
          <MatchWidget matchData={appConfig.matchData} />
        )}
        ...
        {appConfig.eventStreams.length > 0 && appConfig.eventStreams.map((ev: EventStream) => (
          <TouchableOpacity
            key={ev.id}
            activeOpacity={0.85}
            onPress={() => router.push({ pathname: '/event-live', params: { url: ev.streamUrl, label: ev.label } } as any)}
            style={styles.eventSlot}
          >
            <View style={styles.liveSectionHeader}>
              <Animated.View style={[styles.liveDot, { transform: [{ scale: livePulseAnim }] }]} />
              <Radio color={Colors.dark.text} size={16} />
              <Text style={styles.liveSectionTitle}>{ev.label}</Text>
            </View>
            <View style={styles.eventSlotBanner}>
              <Image
                source={ev.bannerImageUrl ? { uri: ev.bannerImageUrl } : require('@/assets/images/live-banner.jpg')}
                style={styles.liveBannerImage}
                contentFit="cover"
                cachePolicy="memory-disk"
              />
              <LinearGradient
                ...
                style={styles.liveBannerOverlay}
              />
              <View style={styles.eventSlotPlay}>
