...
export default function LiveScreen() {
  ...
  const pauseBehindTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (pauseBehindTimerRef.current) clearTimeout(pauseBehindTimerRef.current);
    };
  }, []);
  ...
  const player = useVideoPlayer(
    { uri: appConfig.liveStreamUrl, headers: { Referer: 'LiraTVPlay' } },
    (p) => {
      p.loop = false;
  ...
  useEffect(() => {
    if (playerStatus === 'error') {
      ...
      captureWarn(new Error(errMsg), 'live', 'player_status_error');
      setIsStalled(false);
      // Se offline: mostra subito errore, niente auto-retry
      ...
      if (autoRetryCountRef.current <= MAX_AUTO_RETRIES) {
        // Auto-retry silenzioso: mostra spinner per 6s poi riprova
        console.log(`[Live] Auto-retry ${autoRetryCountRef.current}/${MAX_AUTO_RETRIES} in 6s...`);
        ...
        // Auto-retry esaurito: mostra schermata errore manuale
        console.log('[Live] Auto-retry esaurito, mostra errore manuale');
        setStreamError(true);
        setIsAutoRetrying(false);
      }
    } else if (playerStatus === 'readyToPlay') {
      console.log('[Live] Ready to play');
      // Connessione ripristinata: azzera contatore auto-retry
  ...
  }, [isConnected, streamError, isStalled, isAutoRetrying, player]);
  ...
  useEffect(() => {
    if (!deviceRegisteredRef.current) {
      ...
      void getDeviceId().then((deviceId) => {
        ...
        registerDevice(deviceId, Platform.OS, String(Platform.Version), Constants.expoConfig?.version ?? '').catch(() => console.log('[Live] Register device error'));
  ...
  // Heartbeat — invia un ping ogni 30s quando il live è in riproduzione
  useEffect(() => {
    if (!isPlaying || !deviceIdRef.current) return;
    const sendHeartbeat = () => {
      fetch(`${getBaseUrl()}/api/heartbeat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ deviceId: deviceIdRef.current }),
      }).catch(() => {}); // silenzioso in caso di errore
    };
    sendHeartbeat(); // primo ping immediato
  ...
  const handlePlayPause = () => {
    if (isPlaying) {
      player.pause();
      if (pauseBehindTimerRef.current) clearTimeout(pauseBehindTimerRef.current);
      pauseBehindTimerRef.current = setTimeout(() => {
        if (isMountedRef.current) setIsLiveBehind(true);
      }, 5000);
    } else {
      if (pauseBehindTimerRef.current) {
        clearTimeout(pauseBehindTimerRef.current);
        pauseBehindTimerRef.current = null;
      }
      player.play();
    }
  };
    ...
    <View style={containerStyle}>
      ...
      {!isFullscreen && (
        <View style={styles.headerContainer}>
          <View style={styles.headerContent}>
            <View style={styles.inOndaBadge}>
              <Text style={styles.inOndaText}>In Onda</Text>
            </View>
            {currentProgram && (
      ...
      <View style={videoContainerStyle}>
        ...
        {showControls && !isLoading && (
          <>
            <View style={[styles.topControls, isFullscreen && styles.topControlsFullscreen, { paddingTop: isFullscreen ? Math.max(insets.top, 16) : 8 }]}>
              {isFullscreen && (
            ...
            {!streamError && !isStalled && (
              isLiveBehind ? (
                <TouchableOpacity
                  style={[styles.liveBadge, isFullscreen && { bottom: Math.max(insets.bottom + 16, 24), left: 16 }]}
                  onPress={() => {
                ...
                >
                  ...
                  <Text style={styles.liveBadgeText}>TORNA AL LIVE</Text>
                </TouchableOpacity>
              ) : (
                <View style={[styles.liveBadge, isFullscreen && { bottom: Math.max(insets.bottom + 16, 24), left: 16 }]}>
