import { VideoContent, ProgramSchedule, NewsArticle } from '@/types';
...
const NEWS_CACHE_KEY = 'cached_news_articles_v1';
...
function extractFromHTML(html: string, selector: string): string | null {
  ...
  if (selector === '#video-url') {
    const match = html.match(/<div[^>]*id=["']video-url["'][^>]*>([^<]*)<\/div>/i);
...
async function fetchAllFromSupabase(): Promise<VideoContent[]> {
  try {
    ...
    console.warn('🔵 [Supabase Direct] fetchAllFromSupabase error:', e instanceof Error ? e.message : String(e));
    return [];
  }
}
...
function refreshInBackground() {
  fetchAllFromSupabase().then(async (supabaseVideos) => {
    if (supabaseVideos.length > 0) {
      ...
      console.log('🔄 [Background] Updated local cache from Supabase Direct');
    }
  }).catch((err) => {
...
function refreshFromRSSInBackground(currentVideos: VideoContent[]) {
  fetchAllFromRSS().then(async (rssVideos) => {
    ...
    if (merged.length > currentVideos.length) {
      console.log(`🔄 [Background] RSS found ${merged.length - currentVideos.length} new videos, updating cache`);
      await AsyncStorage.setItem(CACHE_KEY, JSON.stringify({
        data: merged,
        timestamp: Date.now()
      }));
...
async function fetchScheduleFromSupabaseDirect(): Promise<{ programs: ProgramSchedule[]; lastUpdated: string | null }> {
  ...
  if (error) {
    console.warn('🔵 [Supabase Direct] cached_schedule error:', error.message);
...
export async function fetchFeaturedPrograms(): Promise<FeaturedProgram[]> {
  try {
    ...
    console.warn('🔵 [Featured] Error:', e instanceof Error ? e.message : String(e));
    return [];
  }
}
...
export async function fetchActivePolls(deviceId: string): Promise<PollData[]> {
  try {
    ...
    console.warn('🔵 [Polls] Error:', e instanceof Error ? e.message : String(e));
    return [];
  }
}
...
export async function submitPollVote(pollId: string, optionIndex: number, deviceId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.warn('🔵 [Poll Vote] Error:', error.message);
      return false;
    }
    ...
    console.warn('🔵 [Poll Vote] Error:', e instanceof Error ? e.message : String(e));
    return false;
  }
}
...
export async function fetchViewCounts(videoIds: string[]): Promise<Record<string, number>> {
  ...
  try {
    ...
    if (error) {
      console.warn('🔵 [Views] getCounts error:', error.message);
      return {};
    }
    ...
    console.warn('🔵 [Views] Error:', e instanceof Error ? e.message : String(e));
    return {};
  }
}
...
export async function trackVideoView(videoId: string, deviceId: string): Promise<number> {
  try {
    ...
    console.warn('[Views] trackVideoView error:', e instanceof Error ? e.message : String(e));
    return 0;
  }
}
