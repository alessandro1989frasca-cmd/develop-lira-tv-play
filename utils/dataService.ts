...
const RSS_COOLDOWN_MS = 15 * 60 * 1000;
let _lastRSSFetch = 0;
...
function mapArticoliCategory(categoriesLower: string[]): VideoContent['category'] {
  if (categoriesLower.some(c => c.includes('liratg') || c.includes('lira tg'))) return 'edizioni_tg';
  if (categoriesLower.some(c => c.includes('sport'))) return 'sport';
  if (categoriesLower.some(c => c.includes('cronaca'))) return 'cronaca';
  if (categoriesLower.some(c => c.includes('politica'))) return 'politica';
  if (categoriesLower.some(c => c.includes('cultura'))) return 'cultura';
  if (categoriesLower.some(c => c.includes('ambiente'))) return 'ambiente';
  return 'all';
}

function parseRSSItemObj(item: any, feedSource: 'main' | 'programmi' | 'news' | 'sport' | 'cronaca' | 'politica' | 'cultura' | 'ambiente' | 'articoli'): VideoContent | null {
  try {
    ...
    if (feedSource === 'main') {
        ...
        return video;
    } else if (feedSource === 'articoli') {
        video.category = mapArticoliCategory(categoriesLower);
        return video;
    }
...
async function fetchSingleFeed(feedUrl: string, feedName: string, feedSource: 'main' | 'programmi' | 'news' | 'sport' | 'cronaca' | 'politica' | 'cultura' | 'ambiente' | 'articoli', limit?: number): Promise<VideoContent[]> {
  try {
...
async function fetchAllFromRSS(): Promise<VideoContent[]> {
  ...
  const [programmiVideos, articoliVideos] = await Promise.all([
    fetchSingleFeed('https://www.liratv.it/feed/programmi-app/', 'Programmi App', 'programmi'),
    fetchSingleFeed('https://www.liratv.it/feed/articoli-app/', 'Articoli App', 'articoli'),
  ]);

  const allVideos = [...programmiVideos, ...articoliVideos];
...
function refreshFromRSSInBackground(currentVideos: VideoContent[]) {
  const now = Date.now();
  if (now - _lastRSSFetch < RSS_COOLDOWN_MS) {
    console.log(`🔄 [Background] RSS cooldown attivo, skip (ultimo fetch ${Math.round((now - _lastRSSFetch) / 1000)}s fa)`);
    return;
  }
  _lastRSSFetch = now;
...
export async function registerDevice(deviceId: string, platform?: string, osVersion?: string, appVersion?: string): Promise<void> {
  try {
    ...
    if (appVersion) platformData.app_version = appVersion;
...
const NEWS_CLEANUP_KEY = 'news_last_cleanup';

async function cleanupOldNewsIfNeeded(): Promise<void> {
  try {
    const lastCleanup = await AsyncStorage.getItem(NEWS_CLEANUP_KEY);
    if (lastCleanup && Date.now() - parseInt(lastCleanup, 10) < 24 * 60 * 60 * 1000) return;
    ...
    const { error, count } = await supabase
      ...
      .lt('pub_date', cutoff);
    if (error) {
      console.warn('⚠️ [News] Cleanup failed:', error.message);
    } else {
      if (count && count > 0) console.log(`🗑️ [News] Removed ${count} articles older than 7 days`);
      await AsyncStorage.setItem(NEWS_CLEANUP_KEY, String(Date.now()));
    }
  } catch (e) {
    console.warn('⚠️ [News] Cleanup error:', e);
  }
}

async function syncNewsToSupabase(articles: NewsArticle[]): Promise<void> {
  ...
  try {
    const { data: freshCheck } = await supabase
      ...
      .limit(1)
      .single();
    if (freshCheck?.updated_at) {
      const ageMs = Date.now() - new Date(freshCheck.updated_at).getTime();
      if (ageMs < 60 * 60 * 1000) {
        console.log('⏭️ [News] Supabase sync skipped (già aggiornato da un altro dispositivo)');
        return;
      }
    }
    const now = new Date().toISOString();
    const rows = articles.slice(0, 50).map(a => ({
      id: a.id,
      title: a.title,
      link: a.link,
    ...
    if (error) {
      console.warn('⚠️ [News] Supabase sync failed:', error.message);
    } else {
      console.log(`✅ [News] Synced ${rows.length} articles to Supabase`);
      void cleanupOldNewsIfNeeded();
    }
  } catch (e) {
    console.warn('⚠️ [News] Supabase sync error:', e);
  }
}

async function fetchNewsFromSupabase(): Promise<NewsArticle[]> {
  try {
    const { data, error } = await supabase
      .from('cached_news')
      .select('*')
    ...
    console.log(`📦 [News] Loaded ${data.length} articles from Supabase fallback`);
    ...
    console.warn('⚠️ [News] Supabase fallback error:', e);
    return [];
  }
}
...
export async function fetchNewsArticles(): Promise<NewsArticle[]> {
  try {
    ...
    if (articles.length === 0) {
      console.warn('⚠️ [News] WordPress returned 0 articles (maintenance?), trying Supabase fallback...');
      const supabaseFallback = await fetchNewsFromSupabase();
      if (supabaseFallback.length > 0) return supabaseFallback;
      try {
        const cached = await AsyncStorage.getItem(NEWS_CACHE_KEY);
        if (cached) {
          const { data } = JSON.parse(cached);
          console.log('📦 [News] Using AsyncStorage cache (0 articles from RSS)');
    ...
    void syncNewsToSupabase(articles);
    ...
    const supabaseArticles = await fetchNewsFromSupabase();
