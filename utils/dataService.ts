...
function refreshInBackground() {
  fetchAllFromSupabase().then(async (supabaseVideos) => {
    if (supabaseVideos.length > 0) {
      ...
      const existingCount = existing ? JSON.parse(existing).data?.length ?? 0 : 0;
      await AsyncStorage.setItem(CACHE_KEY, JSON.stringify({
        data: supabaseVideos,
        timestamp: Date.now()
      }));
      if (supabaseVideos.length > existingCount) {
        _queryClient?.setQueryData(['videos'], supabaseVideos);
        console.log(`🔄 [Background] UI updated: ${supabaseVideos.length - existingCount} new videos from Supabase`);
      } else {
...
function refreshFromRSSInBackground(currentVideos: VideoContent[]) {
  fetchAllFromRSS().then(async (rssVideos) => {
    ...
    if (merged.length > currentVideos.length) {
      const newCount = merged.length - currentVideos.length;
      console.log(`🔄 [Background] RSS found ${newCount} new videos, updating cache + UI`);
      await AsyncStorage.setItem(CACHE_KEY, JSON.stringify({
        data: merged,
        timestamp: Date.now()
      }));
